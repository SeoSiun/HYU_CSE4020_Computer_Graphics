...
def render():
    ...
    M = np.identity(4)
    Rfz = np.array([[np.cos(fzang), -np.sin(fzang), 0],
                   [np.sin(fzang), np.cos(fzang), 0],
                   [0,0,1]])
    Rx = np.array([[1,0,0],
                   [0, np.cos(xang), -np.sin(xang)],
                   [0, np.sin(xang), np.cos(xang)]])
    Rsz = np.array([[np.cos(szang), -np.sin(szang), 0],
                   [np.sin(szang), np.cos(szang), 0],
                   [0,0,1]])
    M[:3,:3] = Rfz @ Rx @ Rsz
    glMultMatrixf(M.T)
...
def main():
    global gVertexArraySeparate,a,b,r
    a=0
    b=0
    r=0
